...
class RPEModule(nn.Module):
    def __init__(self, embed_dim, num_heads=8, num_levels=4, num_points=4):
        super().__init__()
        self.num_heads = num_heads
        self.num_levels = num_levels
        self.num_points = num_points

        self.cpb_mlp1 = self.build_cpb_mlp(2, embed_dim, num_heads*2)
        self.cpb_mlp2 = self.build_cpb_mlp(2, embed_dim, num_heads*2)
    ...
    def forward(self, reference_points):
        """
        Args:
            reference_points是多层，需要分层处理
            relative_offsets: Tensor [B, L_q, num_levels, num_points, 2].
        Returns:
            relative_position_embedding: Tensor [B, L_q, num_heads, num_levels, num_points, 2].
        """
        bs, Len_q, _, _ = reference_points.shape
        ref_pts = torch.cat([
            reference_points[:, :, :, :2] - reference_points[:, :, :, 2:] / 2,
            reference_points[:, :, :, :2] + reference_points[:, :, :, 2:] / 2,
        ], dim=-1)
        eps = 1e-2
        pos_x = torch.linspace(eps, 1 - eps, 40, dtype=torch.float32, device=ref_pts.device)[None, None, :, None]
        pos_y = torch.linspace(eps, 1 - eps, 40, dtype=torch.float32, device=ref_pts.device)[None, None, :, None]  # 1, 1, h, 1
        delta_x = ref_pts[..., 0::2] - pos_x  # B, nQ, w, 2
        delta_y = ref_pts[..., 1::2] - pos_y  # B, nQ, h, 2
        # 采用二维解耦方式，计算量显著降低7G
        rpe_x, rpe_y = self.cpb_mlp1(delta_x), self.cpb_mlp2(delta_y) # B, nQ, w/h, nheads * 2
        rpe_level = (rpe_x[:, :, None] + rpe_y[:, :, :, None]).flatten(2, 3)  # B, nQ, h, w, nheads * 2 ->  B, nQ, h*w, nheads, 2
        ...
        rpe = torch.gather(
            rpe_level,
            dim=2,
            index=topk_indices.unsqueeze(-1).expand(-1, -1, -1, -1, rpe_level.size(-1))
        )

        rpe = rpe.reshape(bs, Len_q, self.num_points, self.num_heads, 2).permute(0, 1, 3, 2, 4)
        return rpe

class MSGlobalDeformableAttention(nn.Module):
    def __init__(
            self,
            embed_dim=256,
            num_heads=8,
        ...
        """
        super(MSGlobalDeformableAttention, self).__init__()
        ...
        self.rpe_module = RPEModule(embed_dim, num_heads, num_levels, sum(num_points_list))
    ...
    def forward(self,
        ...
        rpe = self.rpe_module(reference_points)
        sampling_offsets: torch.Tensor = self.sampling_offsets(query)
        sampling_offsets = sampling_offsets.reshape(bs, Len_q, self.num_heads, sum(self.num_points_list), 2) + rpe
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        # self.cross_attn = MSGlobalDeformableAttention(d_model, n_head, n_levels, n_points, method=cross_attn_method)
